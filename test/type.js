"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const assert = require("assert");
const index_1 = require("../index");
const utils_1 = require("../src/utils");
describe('basic type', () => {
    it('GeneralizedTime', () => {
        const dt = '2020-12-23T03:27:57.000Z';
        const ber = '180F32303230313232333033323735375A'.toLowerCase();
        index_1.Value.fromBER(Buffer.from(ber, 'hex')).map(v => {
            assert.ok(v.__value === dt); // test decoder
            index_1.compose(index_1.schema.generalizedTime(), v)
                .map(v => assert.ok(v === dt)); // test schema
        });
        assert.ok(index_1.dumper.generalizedTime(new Date(Date.parse(dt))).toString('hex') === ber); // test dumper
    });
    it('integer', () => {
        const pkeySchema = index_1.schema.seq().object({
            n1: index_1.schema.int(),
            n2: index_1.schema.int(),
        });
        // evernote public key
        const pkey = '';
        index_1.Value.fromBER(Buffer.from(pkey, 'hex'))
            .chain(v => index_1.compose(pkeySchema, v))
            .map((v) => {
            assert.ok(v['n1'].startsWith('0xA49FD2A331160B6F9498B51C6346C1CC6A8D8F0C14C376DC09D692E0531141'.toLowerCase()));
            assert.ok(BigInt(v['n1']).toString().startsWith('207818935757469208598194479909767130890755013425177397290481657858356'));
            assert.ok(v['n2'] === 65537);
            const n1size = utils_1.LengthBlock.value(Buffer.from('820101', 'hex'));
            const pkey2 = index_1.dumper.seq([
                index_1.dumper.integer(v['n1'], n1size),
                index_1.dumper.integer(65537),
            ]);
            assert.ok(pkey2.toString('hex') === pkey);
        });
    });
});
